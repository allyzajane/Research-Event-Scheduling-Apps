...
import { Upload, Search, Trash2, FileText, File, Image, Sheet, MoreHorizontal, Download } from "lucide-react";
...
export default function DocumentsPage() {
  ...
  const onDrop = useCallback((files: File[]) => {
    if (files[0]) { setPendingFile(files[0]); setUploadTitle(files[0].name.replace(/\.[^.]+$/, "")); }
  }, []);
  ...
  const handleUpload = async () => {
    ...
    try {
      ...
      const res = await fetch("/api/documents/upload", {
        ...
        body: JSON.stringify({ file_base64: base64, file_name: pendingFile.name, mime_type: pendingFile.type, title: uploadTitle || pendingFile.name, description: uploadDesc || null })
      });
      if (!res.ok) throw new Error(await res.text());
      invalidate(); setUploadOpen(false); setPendingFile(null); setUploadTitle(""); setUploadDesc("");
    } catch (err) {
    ...
    <div className="p-6 space-y-6 max-w-7xl mx-auto">
      ...
      {isLoading ? (
        ...
        <div className="text-center py-20 text-muted-foreground">
          ...
          <Button variant="outline" onClick={() => setUploadOpen(true)} className="mt-3">{t("documents.uploadDocument")}</Button>
        </div>
      )}
